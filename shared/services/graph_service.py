...
def generate_pie_graph(roll_no, exam_id, class_value, sec):
    try:
        ...
        if not mark:
            raise ValueError("No marks found.")

        if any(m.marks == 0 for m in mark):
            raise ValueError(
                "Enter the marks for all subjects to display the graph."
            )
        ...
        graph_image, exam = generate_pie_graph(...)

        if graph_image is None:
            return render_template(
                "Error.html",
                data="Enter the marks for all subjects to display.",
                location="/home",
            )
